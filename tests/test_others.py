...
def JoinTestCase(TestCase):
    def setUp(self):
        ...
        self.weed = Product.insert(name="weed", owner_id=self.tom.id, detail={})
        self.smoke = Product.insert(name="smoke", owner_id=self.tom.id, detail={})
        self.book = Product.insert(name="book", owner_id=self.jerry.id, detail={})
    ...
    def test_joins(self):
        ...
        with Join(
            Student, "INNER JOIN", Product, "students.id = products.owner_id"
        ) as join:
            join.execute()
            ret = join.fetchall()
        ...
        with Join(
            ...
            "INNER JOIN",
            Product,
            "students.id = products.owner_id",
            "INNER JOIN",
            Car,
            "students.id = cars.owner_id",
        ) as join:
            join.execute()
            ret = join.fetchall()
        assert len(ret) == 4
