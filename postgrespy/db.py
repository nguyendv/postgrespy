...
- Don't forget to commit the connection (`conn.commit()`) and
release it: `pool.putconn(conn)`
"""
...
def get_pool(
    ...
    """ `db.py` assumes that you use only one Postgresql database server for your
    application.
    ...
    if _pool is None:
        ...
        """
        # TODO: New pool is created. Should it be created often?
        if minconn is None:
